...
bool apiHttpPost(const char* apiURL, String postBody){
  bool success=false;
  ...
  if (http.begin(client, apiURL)) {  // HTTP
    ...
    // http.addHeader("Content-length", String(postBody.length()));
    // http.addHeader("Host", "ilxgxw0o3a.execute-api.ap-northeast-1.amazonaws.com\r\n");    
    // start connection and send HTTP header
...
void setup() {
  ...
  body1 = "{\"params\":{\"token_id\":\"rfid_token\",\"login_dict\":{";
  //WiFi.mode(WIFI_STA);

  Serial.print("ESP8266 Board MAC Address:  ");
  Serial.println(WiFi.macAddress());
  bodyMac = "\"wifi_mac\":\"" + String(WiFi.macAddress() + "\"}}}");
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if ( ! mfrc522.PICC_IsNewCardPresent()) {
      ...
      String D_10 = dump_byte_array(mfrc522.uid.uidByte, mfrc522.uid.size); // 顯示卡片的UID
      Serial.println();

      postBody = body1 + "\"rf_id\":\"" + String(D_10)+ "\"," + bodyMac;
      Serial.println(postBody);
      Serial.println(postBody.length());
      ...
      for (int i=0; i<3; i++){
        lastTime = millis();
        //if(apiHttpPost(apiURL, postBody)) {
        if(apiHttpGet(apiURL)) {          
          Serial.printf("delay %d\n", millis() - lastTime);
          break;
        }
